...
// effect 栈
const effectStack = []
// effect 函数用于注册副作用函数,当options.lazy为true时，副作用函数不会立即执行
...
function computed(getter) {
  // value 用来缓存上一次计算的值
  let value
  // dirty 标志，用来标志是否需要重新计算
  let dirty = true

  const effectFn = effect(getter, {
    lazy: true,
    // 添加调度器，在调度器中将dirty 设置为true
    scheduler() {
      if (!dirty) {
        dirty = true
  ...
  const obj = {
    get value() {
      ...
      if (dirty) {
        value = effectFn()
        // 将dirty 设置为false， 下一次访问直接使用缓存到 value 中的值
        dirty = false
      }
      // 读取 value 时，手动调用 track 函数进行跟踪
      track(obj, 'value')
      return value
    }
  }
